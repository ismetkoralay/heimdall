...
func TestLoadModelProviderConfig(t *testing.T) {
	tests := []struct {
		...
		fileToWrite                 string
		fileContent                 string
		expectedModelProviderConfig map[string]ModelProviderConfig
		expectedProviderMap         map[string]string
		expectedErr                 error
	}{
		{
		...
		},
		{
			...
			fileToWrite:                 "models.yaml",
			fileContent:                 validFileContent,
			expectedModelProviderConfig: nil,
			expectedProviderMap:         nil,
  ...
  - name: test_model
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			modelProviderConfig, providerMap, err := LoadModelProviderConfig()
			...
			assert.Equal(t, tt.expectedModelProviderConfig, modelProviderConfig)
			assert.Equal(t, tt.expectedProviderMap, providerMap)
		})
	}
}
